...
// Validamos el parámetro en la URL
if (!$id) {
...
$vendedores      = [];
while ($item = mysqli_fetch_assoc($resVendedores)) {
   $vendedores[] = $item;
}
$vendedorId = $propiedad[ 'fk_id_vendedores' ];
...
if ($_SERVER[ 'REQUEST_METHOD' ] === 'POST') {
   ...
   if (empty($errores)) {
      ...
      // Verificamos si se subió una nueva imagen y si hay una nueva borramos la anterior
      if ($imagen[ 'name' ]) {
         // Eliminamos la imagen anterior
         unlink($folderImage . $propiedad[ 'imagen' ]);
      } else {
         // Conservamos la misma imagen de la propiedad
      ...
      $query = "UPDATE propiedades SET titulo = '$titulo', precio = '$precio', imagen = '$imageName', descripcion = '$descripcion',
                habitaciones = '$habitaciones', wc = '$wc', estacionamiento = '$estacionamiento', fecha_creacion = '$creado',
                fk_id_vendedores = '$vendedorId' WHERE id_propiedades = '$id'";
